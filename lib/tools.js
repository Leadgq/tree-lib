...
/**
 *  @description 向字符添加前后缀
 *  @param { Object } options
 *  @param { String } options.str
 *  @param { String } options.prevOrSuffix
 *  @param { Number } options.len  
 *  @param { String } options.direction
 *  @returns { String }
 *  @example padStrPrevOrSuffix({str:'1',prevOrSuffix:'0',len:4}) ==> '0001'
 *  @example padStrPrevOrSuffix({str:'123',prevOrSuffix:'0',len:5,direction:'suffix'}) ==> '12300'
 **/
export const padStrPrevOrSuffix = (options) => {
    const { str = '', prevOrSuffix = '0', len = 2, direction = "prev" } = options;
    if (!isAbleString(str)) {
        throw new Error(`params str muse be string now type is ${getTypeName(str)}`);
    }
    const strLength = str.length;
    if (strLength >= len) return str;
    const needPadLength = len - strLength;
    if (direction === "prev") {
        return str.padStart(len, prevOrSuffix.repeat(needPadLength))
    } else {
        return str.padEnd(len, prevOrSuffix.repeat(needPadLength))
    }
}

/**
 *  @description 挑选对象中的数组
 *  @param { Array } arr
 *  @param { string } path
 *  @returns { Array }
 *  @example pickArray(arr, 'labelList.address.children')
 *  @description 最后一个是被挑选的数组
 */
// export function pickObjectArray(arr, path) {
//     const arrLength = arr.length;
//     let needLoop = 0;
//     let keys = path.split('.');
//     const copyKeys = window.structuredClone(keys);
//     let result = []
//     function loop(loopArray) {
//         const currentKey = keys.shift();
//         for (const item of loopArray) {
//             const currentValue = item[currentKey] || [];
//             if (Array.isArray(currentValue) && currentKey === copyKeys.at(-1)) {
//                 if (needLoop < arrLength * 2) {
//                     keys = window.structuredClone(copyKeys).slice(1, copyKeys.length);
//                 }
//                 needLoop++;
