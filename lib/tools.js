 ...
 */
export const futureTimestamp = (time, type = 'second') => {
    if (!isAbleNumber(time)) {
        throw new Error(`time muse be number`);
    }
    const timeMap = {
        day: 24 * 60 * 60 * 1000,
        hour: 60 * 60 * 1000,
    ...
    if (!timeMap[type]) {
        throw new Error(`type muse be day | hour | minute | second`);
    }
    return nowTimeStamp() + time * timeMap[type];
}
 ...
 * @param type { 'day' | 'hour' | 'minute' | 'second' }
 */
export const differenceTimestamp = (time, type = 'second') => futureTimestamp(time, type) - nowTimeStamp()
 ...
 * @description 如果不传入key，则认为是基本类型数组
 */
export const fuzzySearch = (arr, keyWord, attribute) => {
    if (!isAbleArray(arr)) {
        throw new Error(`arr muse be array`);
    }
    ...
    return arr.filter(item => reg.test(item[attribute]));
}
