 ...
 */
export const compareStr = (s1, s2, stringIdentifier = '-') => {
    if (!isAbleString(s1) || !isAbleString(s2)) {
        throw new Error(`s1 and s2 muse be string`);
    }
    const it1 = wake(s1, stringIdentifier);
    const it2 = wake(s2, stringIdentifier);
    let res1 = it1.next();
    let res2 = it2.next();
    while (!res1.done && !res2.done) {
        if (Number(res1.value) > Number(res2.value)) {
            return 1;
        } else if (Number(res1.value) < Number(res2.value)) {
            return -1;
        } else {
            res1 = it1.next();
            res2 = it2.next();
...
function* wake(str, stringIdentifier) {
    let part = '';
    for (let i = 0; i < str.length; i++) {
        if (str[i] === stringIdentifier) {
            yield part;
            part = '';
        } else {
            part += str[i];
