 ...
 * @returns {Array} 返回路径
 */
export const findPath = (treeArray, target, key, showDetail, isIncludeMyself = false) => {
...
export const isChildNode = (treeData, node, key) => {
    if (!isAbleArray(treeData) && !isAbleObject(treeData)) {
        console.warn(` param treeData is  ${treeData} is not a array or ${treeData} is empty`);
        return false;
    }
    if (!isAbleObject(node)) {
        console.warn(`param node is ${node} is not a object`);
        return false;
    }
    if (!isAbleString(key)) {
        console.warn(`param  key is ${key} is not a string`);
        return false;
    }
    ...
    if (node[key]) {
        ...
        console.warn(`The value obtained from the current object is empty 、 please check the key value of the current object `);
        return false;
    }
}
...
export const redoTreeData = (tree, handlerTreeFn) => {
    if (!isAbleArray(tree) && !isAbleObject(tree)) {
        console.warn(` param treeData is  ${tree} is not a array or tree is empty`);
        return [];
    }
