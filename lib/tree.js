 ...
 * @param { Array } tree
 * @param  { Function } fn
 * @description 得到每次遍历的节点、广度优先遍历
 */
export const breadthFirstSearch = (tree, fn) => {
    const firstStack = flattenTree(tree)
    if (!isAbleFn(fn)) {
        throw new Error(`param fn is ${fn} is not a function`);
    }
    for (const node of firstStack) {
        fn(node);
    }
}
 ...
 */
export const postOrderFirstSearch = (tree, fn) => {
    const postStack = flattenTreeByPostOrder(tree)
    if (!isAbleFn(fn)) {
        throw new Error(`param fn is ${fn} is not a function`);
    }
    for (const node of postStack) {
        fn(node);
    }
}
...
export const findCurrentNodeBrotherNode = (treeData, node, key, isShowDetail = true) => {
    ...
    if (isAbleArray(parentList)) {
        ...
        if (tree.length > 1) {
            ...
            console.warn(`The current node ${target} does not have a brother node`);
            return [];
        }
    }
}
