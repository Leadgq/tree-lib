...
import { checkTreeParamIsExist, cloneTree } from "./tree-check"
/**
 ...
 * @returns {Array} 返回路径
 */
export const findPath = (tree, target, key, showDetail, isIncludeMyself = false) => {
    const treeData = cloneTree(tree);
    let depPath = '';
    ...
    //  处理结果
    if (depPath) {
        let depPathArray = depPath.split('->');
        const length = depPathArray.length;
        if (length === 1) {
            depPath = depPathArray[0];
        } else {
            if (!isIncludeMyself) depPathArray.pop();
            depPath = depPathArray;
        }
        if (showDetail) {
            depPath = findTreeByFn(treeData, (item) => depPath.includes(item[key]))
        }
    } else {
...
export const isChildNode = (treeData, node, key) => {
    ...
    checkTreeParamIsExist({ tree, node, key });
    if (node?.key) {
...
export const isLeafNode = (treeData, node, key) => {
    ...
    if (node.isLeaf) {
        ...
        const targetNode = findTreeByTreeData(tree, key, node[key]);
        if (targetNode) {
            targetNode.isLeaf = !isAbleArray(targetNode?.children);
...
export const findCurrentNodeBrotherNode = (treeData, node, key, isShowDetail = true) => {
    ...
    const targetNode = findTreeByTreeData(tree, key, node[key]);
    if (targetNode) {
        const parent = findPath(tree, node[key], key);
        if (isAbleArray(parent)) {
            // 直接父节点key
            const directParent = parent.pop();
            const directParentNode = findTreeByTreeData(tree, key, directParent);
            if (!isShowDetail) {
                return directParentNode.children.filter(item => item[key] !== node[key]).map(item => item[key]);
            }
            return directParentNode.children.filter(item => item[key] !== node[key]);
        }
    } else {
        throw new Error(`The value obtained from the current object is empty 、 please check the key value of the current object`);
    }
